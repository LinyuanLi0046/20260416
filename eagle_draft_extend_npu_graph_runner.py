...
class EAGLEDraftExtendNpuGraphRunner(EAGLEDraftExtendCudaGraphRunner):
    ...
    def _replay(self, forward_batch: ForwardBatch):
        supports_graph_cpu_tensor_seq_lens = getattr(
            self.eagle_worker.draft_extend_attn_backend,
            "supports_graph_cpu_tensor_seq_lens",
            False,
        )
        if not is_deepseek_nsa(self.model_runner.model_config.hf_config):
            if supports_graph_cpu_tensor_seq_lens:
