...
class AscendAttnBackend(AttentionBackend):
    ...
    def __init__(self, model_runner: ModelRunner, speculative_step_id: int = 0):
        ...
        # When graph replay can update a persistent CPU seq-lens tensor in place,
        # we can avoid graph.update(cpu_update_input=...), which is fragile when
        # attention is captured on an auxiliary stream.
        self.supports_graph_cpu_tensor_seq_lens = True
    ...
    def _update_graph_seq_lens_cpu_int(
        self,
        ...
        assert metadata.seq_lens_cpu_int is not None

        seq_lens_cpu_int = seq_lens_cpu[: metadata.seq_lens_cpu_int.shape[0]].to(
            dtype=torch.int32,
            device="cpu",
        )
        metadata.seq_lens_cpu_int.copy_(seq_lens_cpu_int)

        if forward_mode.is_target_verify():
            metadata.seq_lens_cpu_int.add_(self.speculative_num_draft_tokens)
        elif forward_mode.is_decode_or_idle() and spec_info is not None:
            metadata.seq_lens_cpu_int.add_(self.speculative_step_id + 1)

        metadata.actual_seq_lengths_kv = metadata.seq_lens_cpu_int
    ...
    def init_forward_metadata_capture_cuda_graph(
        ...
        metadata.seq_lens_cpu_int = torch.empty((bs,), dtype=torch.int32, device="cpu")
        self._update_graph_seq_lens_cpu_int(
            metadata, seq_lens.cpu(), forward_mode, spec_info
        )
        metadata.seq_lens_cpu_list = None
        if (
    ...
    def init_forward_metadata_replay_cuda_graph(
        ...
        self._update_graph_seq_lens_cpu_int(
            metadata, seq_lens_cpu, forward_mode, spec_info
        )
    ...
    def forward_decode_graph(
        ...
        if not self.use_mla:
            ...
            assert self.forward_metadata.seq_lens_cpu_int is not None
...
class AscendAttnMultiStepDraftBackend:
    ...
    def __init__(
        ...
        self.supports_graph_cpu_tensor_seq_lens = all(
            getattr(attn_backend, "supports_graph_cpu_tensor_seq_lens", False)
